...
| 🌿 git | `🌿(main) ✗` | 현재 디렉토리의 git | 브랜치명 + dirty 표시. `+`(초록)=staged, `✗`(빨강)=unstaged. git 레포가 아니면 생략 |
...
| 세그먼트 | 예시 | JSON 소스 | 의미 |
|---------|------|-----------|------|
| 📝 컨텍스트 | `█░░░… 9% 89K/1M` | `context_window.*` | 사용률 막대(너비 15) + % + `사용토큰/컨텍스트크기` |
| ⏳ 5시간 한도 | `5H ███░… 75% (3시간 14분)` | `rate_limits.five_hour.*` | 5시간 사용률 막대(너비 10) + % + 리셋까지 남은 시간. Pro/Max 에서만 |
...
예) 컨텍스트 너비 15, 사용률 9% → `9×15÷100 = 1.35 → 1칸` → `█░░░░░░░░░░░░░░`
예) 5시간 너비 10, 사용률 75% → `75×10÷100 = 7.5 → 7칸` → `███████░░░`
...
| 막대 길이 | `make_bar "$used_int" 15`(컨텍스트), `make_bar "$fh_int" 10`(5H) 의 숫자 |

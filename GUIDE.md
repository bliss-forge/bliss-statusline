...
| 📊 5시간 한도 | `5H ████████░░ 88% (2H 30M)` | `rate_limits.five_hour.*` | 5시간 사용률 막대(너비 10) + % + 리셋까지 남은 시간(`H`/`M`). Pro/Max 에서만 |
...
예) 5시간 너비 10, 사용률 88% → `88×10÷100 = 8.8 → 8칸` → `████████░░`

(소수점은 버림. 그래서 낮은 % 에서는 막대가 1칸 이하로 보일 수 있음.)

> 컨텍스트와 5H 막대 너비는 둘 다 **10**으로 통일돼 있습니다. 더 길게/짧게 원하면 아래 표의 `make_bar` 숫자만 바꾸면 됩니다.
...
| 막대 길이 | `make_bar "$used_int" 10`(컨텍스트), `make_bar "$fh_int" 10`(5H) 의 숫자 |
